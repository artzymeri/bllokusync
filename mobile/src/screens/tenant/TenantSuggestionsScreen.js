...
const TenantSuggestionsScreen = ({ user }) => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      await Promise.all([fetchProperties(), fetchMySuggestions()]);
    } catch (error) {
  ...
  const fetchMySuggestions = async () => {
    try {
      ...
      const data = await response.json();
      // Filter out archived suggestions
      const nonArchivedSuggestions = (data.suggestions || []).filter(
        suggestion => !suggestion.archived || suggestion.archived === false
      );
      setMySuggestions(nonArchivedSuggestions);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    if (!selectedProperty || !title.trim()) {
      Alert.alert('Gabim', 'Ju lutem vendosni titullin e sugjerimit');
      return;
    }

    Alert.alert(
      'Konfirmo',
      'A jeni i sigurt që dëshironi të dërgoni këtë sugjerim?',
      [
        {
          text: 'Anulo',
          style: 'cancel',
        },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              if (response.ok) {
                Alert.alert('Sukses', 'Sugjeri u dërgua me sukses!');
                setTitle('');
                setDescription('');
                await fetchMySuggestions();
              } else {
                Alert.alert('Gabim', data.message || 'Gabim në dërgimin e sugjerimit');
              }
            } catch (error) {
              console.error('Error submitting suggestion:', error);
              Alert.alert('Gabim', 'Gabim në dërgimin e sugjerimit');
            } finally {
              setSubmitting(false);
            }
          },
      ...
      >
        ...
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle}>Dërgo Sugjerim</Text>
          </View>
            ...
            >
              {submitting ? (
                ...
                <>
                  ...
                  <Text style={styles.submitButtonText}>Dërgo Sugjerimin</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
        ...
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle}>Sugjerimet e Mia</Text>
          </View>
          <Text style={styles.cardDescription}>Ndiqni sugjerimet tuaja të dërguara</Text>

          <View style={styles.suggestionsList}>
            {mySuggestions.length === 0 ? (
              <View style={styles.emptyState}>
                <Ionicons name="bulb-outline" size={48} color="#cbd5e1" />
                <Text style={styles.emptyText}>Nuk ka sugjerime të dërguara ende</Text>
              </View>
            ) : (
                  ...
                  <View key={suggestion.id} style={styles.suggestionCard}>
                    <View style={styles.suggestionHeader}>
                      <View style={styles.suggestionInfo}>
                        ...
                        <Text style={styles.suggestionProperty}>
...
export default TenantSuggestionsScreen;
